...
def main():
    ...
    logOutput(dico["INIT"],CHATTER_COLOR_CYAN)
    say(dico["INIT"],CHATTER_LANG)
    CHATTER_BUS.write_byte(CHATTER_ADDRESS_BUS, 1)
    ...
    while True:
        ...
        if event == ord("q") or  event == "q" or  event == "3" :
            ...
            logOutput(dico["TERMINATOR"].format(CHATTER_USERNAME),CHATTER_COLOR_CYAN)
            say(dico["TERMINATOR"].format(CHATTER_USERNAME),CHATTER_LANG)
            #exit(1)
            CHATTER_BUS.write_byte(CHATTER_ADDRESS_BUS, 2)
            time.sleep(1)
